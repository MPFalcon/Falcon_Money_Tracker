...
int setup_driver(config_t * configurations)
{
    ...
    err_code = setup_poll(threadpool,
                          configurations->requested_func,
                          configurations->requested_free_func,
                          configurations->requested_args,
    ...
    {
        DEBUG_PRINT(
            "\n\nERROR [x]  Something went wrong setting up the polling: "
            "%s\n\n",
            __func__);
...
static int server_setup(int * svr_sock, uint16_t port)
{
    ...
    *svr_sock = socket(AF_INET, SOCK_STREAM, IPPROTO_IP);
    ...
    {
        DEBUG_PRINT(
            "\n\nERROR [x]  Failed to ensure address and port can be "
            "reused: %s\n\n",
            __func__);
         ...
         curr_results = curr_results->ai_next)
    {
        ...
        err_code = bind(*svr_sock, curr_results->ai_addr, server_addr_len);
