...
list_t * list_new(FREE_F p_customfree, CMP_F p_customcmp)
{
    ...
    p_new_list->customfree =
        (NULL == p_customfree) ? (FREE_F)custom_free : p_customfree;

    p_new_list->compare_function =
        (NULL == p_customcmp) ? (CMP_F)default_compare : p_customcmp;
...
int list_remove(list_t * p_list, void ** pp_item_to_remove)
{
    ...
    p_deleted_node = (list_node_t *)p_list->compare_function(*pp_item_to_remove,
                                                             p_list->head);
...
list_t * list_find_all_occurrences(list_t * p_list, void ** pp_search_data)
{
    ...
    while ((ERR == check) || (p_curr_node->next != p_list->head))
    {
        p_passed_node = (list_node_t *)p_list->compare_function(*pp_search_data,
                                                                p_curr_node);
        ...
        if (NULL != p_passed_node)
        {
            ...
            if (E_SUCCESS != node_check)
            {
                ...
                p_new_list = NULL;
                print_error("There was an error adding to nodes new list",
                            __func__);
            }
        }
